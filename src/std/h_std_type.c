...
/* value_t h_std_type_ntoc(struct value_t* parameters, size_t args_count) {
    char c;
    int n = snprintf(&c, 1, "%d", H_NFI_NUM_TO_CINT(parameters[0]));
    h_string_t* string = h_string_init_hash(&buffer, n);
    return CHAR_VALUE(string);
} */
...
value_t h_std_type_cton(struct value_t* parameters, size_t args_count) {
    return NUM_VALUE(H_NFI_CHAR_TO_CCHAR(parameters[0]));
}
...
int h_std_type_import(h_locals_stack_t* stack, h_ht_enums_t* enums_table, h_ht_types_t* types_table) {
    ...
    define_native(stack, "type_cton", h_std_type_cton, (value_t[]){(value_t){.type = H_VALUE_CHAR}}, 1, (value_t){.type = H_VALUE_NUMBER});
